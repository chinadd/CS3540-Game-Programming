...
class Node:
    ...
    def insertObject(self, object):
        ...
        if self._depth >= MAX_TREE_DEPTH:
            ...
            return
        elif object[1] + (OBJECT_SIZE/2) < self._bounds[0][1] \
                or object[0] + (OBJECT_SIZE/2) < self._bounds[0][0] \
                or object[1] - (OBJECT_SIZE/2) >= self._bounds[1][1] \
                or object[0] - (OBJECT_SIZE/2) >= self._bounds[1][0]:
            return
        else:
            if self._children is None:
                ...
                center_y = (y0 + y1) / 2
                center_x = (x0 + x1) / 2
                self._children = [Node(((x0, y0), (center_x, center_y)), self._depth + 1),
                                  Node(((center_x, y0), (x1, center_y)), self._depth + 1),
                                  Node(((x0, center_y), (center_x, y1)), self._depth + 1),
                                  Node(((center_x, center_y), (x1, y1)), self._depth + 1)]
    ...
    def findObjects(self, pt):
        ...
        if pt[0] < self._bounds[0][0] \
                or pt[1] < self._bounds[0][1] \
                or pt[0] >= self._bounds[1][0] \
                or pt[1] >= self._bounds[1][1]:
            return []
        else:
            objects = []
            self._visited = True
            ...
            if self._children is None:
                for obj in self._objects:
                    if abs(obj[0] - pt[0]) < (OBJECT_SIZE/2) \
                            and abs(obj[1] - pt[1]) < (OBJECT_SIZE/2):
                        objects.append(obj)

            else:
                for child in self._children:
                    objects = objects + child.findObjects(pt)
