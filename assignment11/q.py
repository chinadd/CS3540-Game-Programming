...
def find_best_action(actions):
    ...
    b_actions, b_q = [], None
    for aa in actions:
        q_at = Q[agent, bonus_present, aa]
        if b_q is None or q_at > b_q:
            b_actions, b_q = [aa], q_at
        elif q_at == b_q:
            b_actions.append(aa)
...
def step_agent(is_learning):
    ...
    if is_learning:
        ...
        if random.random() > EPSILON:
            new_act = find_best_action(actions)
        else:
            new_act = random.choice(actions)
        next_agent, next_bonus_present, reset = transition(agent, bonus_present, new_act)
        r = reward(next_agent, bonus_present)
        max_q = -1e+100
        for aa in actions_for_cell(next_agent):
            max_q = max(max_q, Q[next_agent, next_bonus_present, aa])

        old_q = Q[agent, bonus_present, new_act]
        new_q = (1 - ALPHA) * old_q + ALPHA * (r + GAMMA * max_q)
        Q[agent, bonus_present, new_act] = new_q

    else:
        best_act = find_best_action(actions)
        next_agent, next_bonus_present, reset = transition(agent, bonus_present, random.choice(best_act))
