...
def mouseButton(button, state, mx, my):
    ...
    inver = False
    trans = (mx - root_translate)

    if trans < 0:
        trans = -trans
        inver = True

    if (mx - root_translate)**2 + (SIZE - my - 1)**2 < (ROOT_LENGTH - seg_length)**2:
        return

    if (mx - root_translate)**2 + (SIZE - my - 1)**2 > (seg_length + ROOT_LENGTH)**2:
        return

    temp_root_rotate = (math.atan2((SIZE - my - 1), trans) -
                        math.atan2(
                            seg_length *
                            math.sin(
                                math.acos(
                                    max(-1,
                                        min(1,
                                            (trans ** 2 + (SIZE - my - 1) ** 2 - ROOT_LENGTH ** 2 - seg_length ** 2)
                                            / (2 * ROOT_LENGTH * seg_length)
                                            )
                                        )
                                ))
                            ,
                            ROOT_LENGTH +
                            seg_length *
                            max(-1, min(1, (trans ** 2 + (SIZE - my - 1) ** 2 - ROOT_LENGTH ** 2 - seg_length ** 2)
                                        / (2 * ROOT_LENGTH * seg_length)
                                        )
                                )
                            )
                        ) * 180 / 3.14
    temp_seg_rotate = (math.acos(
                            max(-1,
                                min(1,
                                    (trans ** 2 + (SIZE - my - 1) ** 2 - ROOT_LENGTH ** 2 - seg_length ** 2)
                                    / (2 * ROOT_LENGTH * seg_length)
    ...
    if inver:
        root_rotate = 180 - temp_root_rotate
        seg_rotate = -1 * temp_seg_rotate
    else:
        root_rotate = temp_root_rotate
        seg_rotate = temp_seg_rotate
